...
class YoloService:
    ...
    def add_user(self, image: CustomImage) -> UserRequestStatus:
        ...
        #crop_face_hoto = self.string2Image(self.img2string(faces[0]))

        face_emb = self.face_vec_2(faces[0]) #emb для первого лица из списка "вырезанных" лиц
        ...
        if (hits.points[0].score>=0.88): #лицо с фото сильно похоже на найденное в бд лицо
    ...
    def find_user(self, image: CustomImage) -> UserRequestStatus:
        ...
        face_emb = self.face_vec_2(faces[0])
    ...
    def face_vec_2(self, face: np.ndarray) -> np.ndarray: 
        img = Image.fromarray(face, mode='RGB')
        buffer = io.BytesIO()
        img.save(buffer, format="PNG")

        img_base64 = base64.b64encode(buffer.getvalue()).decode('utf-8')
        face_vector = self.embed_model._get_query_embedding([img_base64])
    ...
    def string2Image(self, string: str) -> CustomImage:
        f = io.BytesIO(base64.b64decode(string.encode('utf-8')))
        pilimage = Image.open(f)

        output_path = "crop_img.png"
        result = pilimage.save(output_path) 
